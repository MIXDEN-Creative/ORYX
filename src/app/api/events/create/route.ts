  ...
  try {
    ...
    const cleanedTicketTypes = ticketTypes
      ...
      .map((ticket, index) => ({
        ...
        price_cents: Math.max(0, Math.round(Number(ticket.price || 0) * 100)),
        quantity: ticket.quantity ? Number(ticket.quantity) : 0,
        description: String(ticket.description || "").trim(),
        sort_order: index,
      }));

    const totalAssigned = cleanedTicketTypes.reduce((sum, ticket) => sum + (ticket.quantity || 0), 0);

    if (totalAssigned > ticketQuantity) {
      return NextResponse.json(
        { ok: false, error: "Total ticket tier quantities cannot exceed event capacity." },
        { status: 400 }
      );
    }
    ...
    if (cleanedTicketTypes.length === 0) {
      return NextResponse.json(
        { ok: false, error: "Add at least one ticket tier." },
        { status: 400 }
      );
    }
    ...
    const { data: event, error: insertError } = await supabaseAdmin
      ...
      .insert({
        ...
        ticket_price: Math.round(basePriceCents / 100),
